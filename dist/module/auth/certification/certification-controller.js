...
const certification_services_1 = require("./certification-services");
class CertificationController {
    /**
     * Award certification to user
     */
    static async awardCertification(req, res) {
        try {
            ...
            const certification = await certification_services_1.CertificationService.awardCertification(userId, courseId, certificateUrl);
     ...
     */
    static async checkAndAwardCertification(req, res) {
        try {
            ...
            const certification = await certification_services_1.CertificationService.checkAndAwardCertification(userId, courseId);
            if (certification) {
                ...
                return res.status(200).json({
                    success: false,
                    message: "Course not completed yet. Please complete all lessons, tests, and quizzes.",
                });
            }
        }
        catch (error) {
    ...
    static async getUserCertifications(req, res) {
        try {
            ...
            const certifications = await certification_services_1.CertificationService.getUserCertifications(userId);
            return res.status(200).json({
                success: true,
                data: certifications,
            });
    ...
    static async getCertificationById(req, res) {
        try {
            ...
            const certification = await certification_services_1.CertificationService.getCertificationById(certificationId);
            if (!certification) {
                return res.status(200).json({
                    success: true,
                    data: null,
     ...
     */
    static async getCourseCertifications(req, res) {
        try {
            ...
            const certifications = await certification_services_1.CertificationService.getCourseCertifications(courseId);
            return res.status(200).json({
                success: true,
                data: certifications,
            });
